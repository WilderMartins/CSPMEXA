...
from huaweicloudsdkiam.v3.model import (
    KeystoneListUsersResponse, UserResult as SdkUserResult, # Para listar usuários
    KeystoneShowUserResponseBody as SdkUserDetailResult, # Para detalhes do usuário
    LoginProtectResult, # Para login_protect
    Credentials as SdkCredentials, # Para chaves de acesso
...
def override_huawei_collector_settings(mock_huawei_settings: Settings):
    with patch('app.huawei.huawei_client_manager._clients_cache', new_callable=dict):
        with patch('app.huawei.huawei_iam_collector.settings', mock_huawei_settings, _if_exists=True), \
...
async def test_get_huawei_iam_users_sdk_error_on_list(mock_huawei_credentials_success_with_domain, mock_iam_client_v3):
    mock_iam_client_v3.keystone_list_users.side_effect = sdk_exceptions.SdkException("IAM.ListError", "Simulated ListUsers SDK failure")
...
async def test_get_huawei_iam_users_one_user_details_and_keys(mock_huawei_credentials_success_with_domain, mock_iam_client_v3):
    ...
    user_list_native = SdkUserResult(id="user-id-1", name="test-iam-user", domain_id="domain_from_creds_123", enabled=True)
    mock_list_users_response = MagicMock(spec=KeystoneListUsersResponse)
    mock_list_users_response.users = [user_list_native]
    mock_iam_client_v3.keystone_list_users.return_value = mock_list_users_response

    # Mock para keystone_show_user (detalhes)
    mock_user_detail_native = SdkUserDetailResult(
        id="user-id-1", name="test-iam-user", domain_id="domain_from_creds_123", enabled=True,
        email="test@example.com", mobile="1234567890",
        login_protect=LoginProtectResult(enabled=True, verification_method="vmfa")
    )
    mock_show_user_response = MagicMock() # O SDK retorna um objeto que tem um atributo 'user'
    mock_show_user_response.user = mock_user_detail_native
...
async def test_get_huawei_iam_users_detail_fetch_error(mock_huawei_credentials_success_with_domain, mock_iam_client_v3):
    user_list_native = SdkUserResult(id="user-err-detail", name="user-err", domain_id="domain_from_creds_123", enabled=True)
    mock_list_users_response = MagicMock(spec=KeystoneListUsersResponse); mock_list_users_response.users = [user_list_native]
    mock_iam_client_v3.keystone_list_users.return_value = mock_list_users_response

    # Simular erro ao buscar detalhes do usuário
    mock_iam_client_v3.keystone_show_user.side_effect = sdk_exceptions.SdkException("IAM.ShowError", "Cannot get user details")
