...
@pytest.mark.asyncio
async def test_create_linked_account_as_admin(client: TestClient, admin_token_headers: dict):
    """
    ...
    with patch("app.services.credentials_service.credentials_service.vault_client") as mock_vault:
        ...
        response = await client.post("/api/v1/accounts/", headers=admin_token_headers, json=account_data)
...
@pytest.mark.asyncio
async def test_get_linked_accounts(client: TestClient, admin_token_headers: dict):
    """
    Testa a listagem de contas vinculadas.
    """
    response = await client.get("/api/v1/accounts/", headers=admin_token_headers)
    assert response.status_code == 200
    assert isinstance(response.json(), list)

@pytest.mark.asyncio
async def test_create_linked_account_as_non_admin_fails(client: TestClient, db_session: Session):
    """
    ...
    response = await client.post("/api/v1/accounts/", headers=headers, json=account_data)

    assert response.status_code == 403
    assert "User with role 'analyst' does not have the required permissions" in response.json()["detail"]
